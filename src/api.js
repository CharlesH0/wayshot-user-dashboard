...
export async function fetchUserProperties(userId) {
  const escaped = userId.replace(/'/g, "\\'");
  const q = `SELECT properties.rc_subscription_status, properties.rc_active_entitlements, properties.$initial_referring_domain, properties.$initial_utm_source, properties.$geoip_country_name, properties.$geoip_country_code FROM persons INNER JOIN person_distinct_ids pdi ON pdi.person_id = id WHERE pdi.distinct_id = '${escaped}' LIMIT 1`;
  const rows = await hogql(q);
  if (rows.length === 0) return null;
  const [status, entitlements, domain, utm, country, countryCode] = rows[0];

  // Get device model from latest event
  const dq = `SELECT properties.$device_model FROM events WHERE distinct_id = '${escaped}' AND properties.$device_model IS NOT NULL ORDER BY timestamp DESC LIMIT 1`;
  const drows = await hogql(dq);
  const device = drows.length > 0 ? drows[0][0] : '';

  return { status, entitlements, domain, utm, country, countryCode, device };
}
